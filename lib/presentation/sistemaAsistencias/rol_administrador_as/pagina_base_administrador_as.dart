...
import 'package:asesorias_fic/presentation/sistemaAsistencias/rol_administrador_as/asistencia/asistencia_screen.dart';
import 'package:asesorias_fic/presentation/sistemaAsistencias/rol_administrador_as/reglas/reglas_screen.dart';
import 'package:asesorias_fic/presentation/sistemaAsistencias/rol_administrador_as/reportes/reportes_screen.dart';
import 'package:asesorias_fic/presentation/sistemaAsistencias/rol_administrador_as/usuarios/usuarios_screen.dart';
import 'package:asesorias_fic/presentation/sistemaTutorias/rol_administrador/reportes.dart';
import 'package:asesorias_fic/presentation/sistemaTutorias/rol_asesor/pagina_base_asesor.dart';
import 'package:flutter/material.dart';

class PaginaBaseAdministradorAs extends StatefulWidget {
  const PaginaBaseAdministradorAs({super.key});

  @override
  State<PaginaBaseAdministradorAs> createState() =>
      _PaginaBaseAdministradorAsState();
}

class _PaginaBaseAdministradorAsState extends State<PaginaBaseAdministradorAs> {
  int _selectedIndex = 0;

  List<Widget> _buildPages(bool esMovil) => [
    AsistenciaScreen(mostrarTitulo: !esMovil),
    UsuariosScreen(mostrarTitulo: !esMovil),
    ReportesScreen(mostrarTitulo: !esMovil),
    ReglasScreen(mostrarTitulo: !esMovil),
  ];

  final List<String> _titles = ['Asistencia', 'Usuarios', 'Reportes', 'Reglas'];

  void _onItemSelected(BuildContext context, int index) {
    if (index == 4) {
      showDialog(context: context, builder: (_) => const AlertaCerrarSesion());
    } else {
