...
class AsesoresDiciplinaresService{
  ...
  Future<List<AsesorDisciplinar>> buscarAsesores(String query) async {
    final response = await http.get(Uri.parse('$baseUrl?p_busqueda=$query'));
  ...
  Future<List<MateriaItem>> obtenerMaterias() async {
    try {
      ...
      if (response.statusCode == 200) {
        final List data = jsonDecode(response.body);
        return data.map((m) => MateriaItem(id: m["id_materia"], nombre: m["materia"])).toList();
      }
    } catch (e) {
  ...
  Future<List<HorarioItem>> obtenerHorarios() async {
    try {
      final response = await http.get(Uri.parse("http://localhost:3000/catalogos/horarios"));
      if (response.statusCode == 200) {
        final List data = jsonDecode(response.body);
        return data.map((h) => HorarioItem(id: h["id_horario"], texto: h["horario"])).toList();
      }
    } catch (e) {
      print("Error en service obtenerHorarios: $e");
    }
    return [];
  }
} */
